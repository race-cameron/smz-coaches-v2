 ...
 *                   tierIndex: 0,          // 0-4: Apprentice/Ranger/Hero/Mystic/Master card skin
...
const CardsState = (() => {
  ...
  // NOTE: card images were briefly routed through IndexedDB
  // (CardImageStore) for higher capacity, but that broke saving
  // entirely for at least one real device (likely a Safari IndexedDB
  // quirk) during a live event, so this is back to storing images
  // directly in the localStorage-backed record — the same approach
  // that was already confirmed working. The IndexedDB migration needs
  // to be revisited with live testing before it ships again.
  function addCard(schoolId, classId, playerId, {
    ...
    tierIndex,         // 0-4, index into the 5 card skins (Apprentice/Ranger/Hero/Mystic/Master)
    ...
    const card = {
      ...
      tierIndex: typeof tierIndex === 'number' ? tierIndex : 0,
      scores: finalScores,
      px, py, pz,
    ...
    const ok = save(data);
    if (!ok) {
      // Roll back the in-memory push so this function never returns a
      // "successful" card object that isn't actually on disk — save()
      // failing silently used to let the wizard think it had saved,
      // navigate away, and lose the card with no warning.
      player.cards.pop();
      throw new Error('Your device is out of storage space for this app. Use "Clear Saved Cards" after downloading a print sheet, then try again.');
    }
    return card;
  }
  ...
  // history untouched. The main way to reclaim localStorage space —
  // use it after downloading a class's print sheet.
  function clearClassCards(schoolId, classId) {
    const data = load();
    const school = data.schools.find(s => s.id === schoolId);
    if (!school) return 0;
    const cls = school.classes.find(c => c.id === classId);
